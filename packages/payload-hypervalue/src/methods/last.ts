...
export type LastArgs = BaseArgs & WithField & WithOptionalId & WithTimeRange

type LastResult = { doc: HypervalueRecord | null }

export const lastMethod = defineMethod<LastArgs, LastResult>({
  build: (discovery, args) => {
    ...
    if (narrowField) {
      return buildNarrowLast(schema, narrowField.tableName, args)
    }
    ...
    if (wideCollection && wideField) {
      return buildWideLast(schema, wideCollection.tableName, wideField.columnName, args)
    }
  ...
  endpoint: {
    path: '/hypervalue/:collection/:field/last',
    method: 'get',
...
function buildNarrowLast(
  schema: string,
  tableName: string,
  args: LastArgs,
): HypervalueDescriptor<LastResult> {
...
function buildWhere(args: LastArgs): SQL {
  const parts: SQL[] = []
  if (args.id) {
